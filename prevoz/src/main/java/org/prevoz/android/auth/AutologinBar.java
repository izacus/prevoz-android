...
public class AutologinBar extends LinearLayout implements OnClickListener, DeviceAccountLogin.AutoLoginCallback
{
    ...
    public void onClick(View v) {
        if (mAutoLoginCancel == v) {
            ...
            if (mAutoLoginHandler != null) {
                ...
                mAutoLoginHandler.login(mAutoLoginAccount.getSelectedItemPosition(), this);
            }
        }
    }

    public void updateAutoLogin(LoginActivity activity, boolean animate)
    {
        DeviceAccountLogin login = activity.getDeviceAccountLogin();
        if (login != null) {
    ...
    void showAutoLogin(boolean animate) {
        mActivity.showAutoLogin(animate);
    }

    void hideAutoLogin(boolean animate) {
        mActivity.hideAutoLogin(animate);
    }
