...
public class LoginStatusTask implements Runnable
{
	...
	public void start(Handler callback)
	{
		...
		statusDialog = ProgressDialog.show(context, null, context
				.getResources().getString(R.string.checking_login_status));
	...
	public void run()
	{
		...
		try
		{
			 ...
			 * context.getSharedPreferences(Globals.PREF_FILE_NAME, 0); String
			...
			String response = HTTPHelper.httpGet(Globals.API_URL
					+ "/accounts/status/", null); // HTTPHelper.buildGetParams(params));
			...
			if (isLoggedIn)
			{
				...
				/*
				 * if (jsonObj.has("apikey")) { apikey =
				 * jsonObj.getString("apikey"); SharedPreferences.Editor editor
				 * = settings.edit(); editor.putString("apikey", apikey);
				 * editor.commit();
				 * 
