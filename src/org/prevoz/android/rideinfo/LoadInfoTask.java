...
public class LoadInfoTask implements Runnable
{
	...
	public void run()
	{
		...
		{
			Log.e(this.toString(), "Error retrieving data for ride ID "
					+ rideID, e);
		...
		try
		{
			...
			result = new Ride(id, type, from, to, time, people, price, author,
					contact, comment, isAuthor);
		}
		catch (JSONException e)
		{
			Log.e(this.toString(), "Error parsing JSON for ride id " + rideID,
					e);
			callback.sendEmptyMessage(Globals.REQUEST_ERROR_SERVER);
			return;
		}
		...
		Log.i(this.toString(), "Succesfully parsed response for ride id "
				+ rideID);
