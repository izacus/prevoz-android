...
public class SearchActivity extends Activity implements OnDateSetListener
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		items[0] = new SearchTypeSpinnerItem(1,
				getString(R.string.searching_shares));
		items[1] = new SearchTypeSpinnerItem(0,
				getString(R.string.searching_seekers));

		Spinner searchTypeSpinner = (Spinner) findViewById(R.id.search_type);

		ArrayAdapter<SearchTypeSpinnerItem> spinnerAdapter = new ArrayAdapter<SearchActivity.SearchTypeSpinnerItem>(
		...
		if (savedInstanceState != null)
		{
			...
			populateSearchForm(savedInstanceState.getString("from_loc"),
					savedInstanceState.getString("to_loc"), selectedDate);

			// Set current view
			currentView = SearchViews.values()[savedInstanceState
					.getInt("current_view")];

			// Select right search type radio button
			searchTypeSpinner.setSelection(savedInstanceState
					.getInt("search_type"));
		}
		else
		{
		...
		// GPS buttons
		((ImageButton) findViewById(R.id.gps_from))
		...
		((ImageButton) findViewById(R.id.gps_to))
				.setOnClickListener(new OnClickListener()
				{
					public void onClick(View v)
					{
	...
	protected void onSaveInstanceState(Bundle outState)
	{
		...
		outState.putString("from_loc",
				((TextView) findViewById(R.id.fromField)).getText().toString());
		outState.putString("to_loc", ((TextView) findViewById(R.id.toField))
				.getText().toString());

		// Store currently selected search type
		outState.putInt("search_type",
	...
	/**
	 * Invoked when user selects new date in search form Sets new date in the
	 * form.
	 */
	...
	private void startSearch()
	{
		...
		parameters.put("f",
				((AutoCompleteTextView) findViewById(R.id.fromField)).getText()
						.toString());
		parameters.put("fc", "SI");
		parameters.put("t", ((AutoCompleteTextView) findViewById(R.id.toField))
				.getText().toString());
		parameters.put("tc", "SI");
		parameters.put(
				"client",
				"android"
						+ StringUtil.numberOnly(
								getString(R.string.app_version), false));

		// Build date
		SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");
		parameters.put("d", formatter.format(selectedDate.getTime()));

		int search_type = ((SearchTypeSpinnerItem) ((Spinner) findViewById(R.id.search_type))
		...
		Handler searchHandler = new Handler()
		{
			...
			public void handleMessage(Message msg)
			{
				...
				case Globals.REQUEST_SUCCESS:
					SearchActivity.getInstance().setSearchResults(
							task.getResults());
					SearchActivity.getInstance().buildResultsList();
					break;

				case Globals.REQUEST_ERROR_SERVER:
					Toast.makeText(SearchActivity.getInstance(),
	...
	private void buildResultsList()
	{
		...
		// Build categories
		if (searchResults.getRides() != null
				&& searchResults.getRides().size() > 0)
		{
			...
			ArrayList<String> ridePaths = new ArrayList<String>(
					ridesByPath.keySet());
			Collections.sort(ridePaths);

			for (String path : ridePaths)
			{
				resultsAdapter.addSection(path, new SearchResultAdapter(
						SearchActivity.getInstance(), ridesByPath.get(path)));
			}
			...
			resultsList.setOnItemClickListener(new OnItemClickListener()
			{
				...
				{
					SearchResultViewWrapper viewWrapper = (SearchResultViewWrapper) view
							.getTag();
					Intent intent = new Intent(SearchActivity.getInstance(),
							RideInfoActivity.class);
					intent.putExtra(RideInfoActivity.RIDE_ID,
							viewWrapper.getRideId());
		...
		else
		{
			...
			ArrayAdapter<String> noResultsAdapter = new ArrayAdapter<String>(
					this, android.R.layout.simple_list_item_1, noResults);
